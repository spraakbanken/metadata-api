...
def renew_cache() -> tuple[Response, int]:
    ...
    # Create config dict and remove keys that can't be JSON serialized
    config_obj = dict(current_app.config)
    for key in ("cache_client", "PERMANENT_SESSION_LIFETIME"):
        config_obj.pop(key, None)
    config_obj["STATIC"] = str(config_obj["STATIC"])
