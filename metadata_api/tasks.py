...
celery_app = Celery("metadata_api", broker="redis://localhost:6379/0")
...
def renew_cache_task(
    ...
    Args:
        config_obj: Configuration object for the application.
    ...
    try:
        repo = Repo(config_obj["METADATA_DIR"])
        ...
        errors.append(msg)
        utils.send_to_slack(msg, config_obj.get("SLACK_WEBHOOK", ""))
        return False
    ...
    if request_method == "POST":
        ...
        try:
            ...
            if payload:
                ...
                file_limit = config_obj["GITHUB_FILE_LIMIT"]
    ...
    try:
        ...
        process_resources(
            resource_paths=resource_paths, config_obj=config_obj, validate=True, debug=debug, offline=offline
        )
        logger.info("Cache renewal task: process_resources completed.")

        cache_client = None
        if not config_obj.get("NO_CACHE") and config_obj.get("cache_client") is not None:
            cache_client = config_obj["cache_client"]
            cache_client.flush_all()
        # Reload resources and resource texts to populate cache
        static_path = Path(config_obj["STATIC"])
        utils.load_resources(config_obj["RESOURCES"], static_path, cache_client=cache_client)
        utils.load_json(
            static_path / config_obj["RESOURCE_TEXTS_FILE"], prefix="res_descr", cache_client=cache_client
        )
        utils.load_json(static_path / config_obj["COLLECTIONS_FILE"], cache_client=cache_client)
        success = True
    ...
    if errors or warnings:
        ...
        utils.send_to_slack(
            "Cache renewal completed.\n" + "\n".join(errors + warnings), config_obj.get("SLACK_WEBHOOK", "")
        )
