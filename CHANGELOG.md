...
- Logging configuration in `gen_pids.py` now uses a filter to send log messages below ERROR to stdout and ERROR and
  above to stderr.
- Fixed a bug in `gen_pids.py` that caused some missing language properties in the Datacite metadata.
...
- `/schema` route (returning the JSON schema for the resource listings generated by the `/` endpoint) has been renamed
  to `/response-schema`.
- The swagger documentation is now available at `/swagger` instead of `/docs`.
...
[unreleased]: https://github.com/spraakbanken/metadata-api/compare/v4.1.0...dev/
[4.1.0]: https://github.com/spraakbanken/metadata-api/releases/tag/v4.1.0
